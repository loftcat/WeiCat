...
import com.loftcat.weibo.bean.CommentVo;
 ...
 * @Version: v1.1
 */
public class CommentsExpandableAdapter extends BaseExpandableListAdapter {
	private Context context;
	ArrayList<CommentVo> commentVos;
	...
	public void reSetData(ArrayList<CommentVo> commentVos) {
		this.commentVos = commentVos;
	}

	public CommentsExpandableAdapter(ArrayList<CommentVo> commentVos,
			Context context) {
		this.commentVos = commentVos;
	...
	public View getChildView(final int groupPosition, int childPosition,
		...
		if (convertView != null) {
			...
			v = ((Activity) context).getLayoutInflater().inflate(
					R.layout.status_item_child2, null);
			...
			cvHolder.status = (TextView) v.findViewById(R.id.status);
		...
		cvHolder.forward.setOnClickListener(new TextView.OnClickListener() {
			...
			public void onClick(View v) {
				...
				intent.putExtra("userID", commentVos.get(groupPosition)
						.getStatus().getUser().getId());
				intent.putExtra("status_id", commentVos.get(groupPosition)
						.getStatus().getId());
		...
		cvHolder.comment.setOnClickListener(new TextView.OnClickListener() {
			...
			public void onClick(View v) {
				...
				intent.putExtra("status_id", commentVos.get(groupPosition)
						.getStatus().getId());
				intent.putExtra("reply_id", commentVos.get(groupPosition)
						.getId());
				intent.putExtra("mode", "reply");
				intent.putExtra("userName", commentVos.get(groupPosition)
						.getUser().getName());
				intent.putExtra("msg", commentVos.get(groupPosition).getText());
				context.startActivity(intent);
			}
		});
		cvHolder.status.setOnClickListener(new TextView.OnClickListener() {
			...
			public void onClick(View v) {
				...
				budBundle.putSerializable("statusVo",
						commentVos.get(groupPosition).getStatus());
				budBundle.putString("from", "comment");
	...
	public View getGroupView(final int groupPosition, boolean isExpanded,
		...
		viewHolder.name.setText(commentVos.get(groupPosition).getUser()
				.getName());
		viewHolder.msg.setText(commentVos.get(groupPosition).getText());
		imageLoader.displayImage(commentVos.get(groupPosition).getUser()
				.getProfile_image_url(), viewHolder.head, options);
		viewHolder.time.setText(TimeUtil.converTime(new Date(commentVos.get(
		...
		viewHolder.source.setText(Html.fromHtml(commentVos.get(groupPosition)
				.getSource()));

		if (commentVos.get(groupPosition).getReply_comment() != null) {
			viewHolder.retweeted_layout.setVisibility(View.VISIBLE);

			viewHolder.retweeted_msg.setText(((commentVos.get(groupPosition)
					.getReply_comment().getUser() == null) ? ""
					: (commentVos.get(groupPosition).getReply_comment()
							.getUser().getName() + ":  "))
					+ (commentVos.get(groupPosition).getReply_comment()
							.getText()));

			// viewHolder.retweeted_replay.setText("回复("
			// + String.valueOf(commentVos.get(groupPosition).getStatus()
			// .getComments_count()) + ")");
			// viewHolder.retweeted_forward.setText("转发("
			// + String.valueOf(commentVos.get(groupPosition).getStatus()
			// .getReposts_count()) + ")");
			viewHolder.retweeted_replay.setVisibility(View.GONE);
			viewHolder.retweeted_forward.setVisibility(View.GONE);

			// if (commentVos.get(groupPosition).getStatus().getThumbnail_pic()
			// != null) {
			// imageLoader.displayImage(commentVos.get(groupPosition)
			// .getStatus().getThumbnail_pic(),
			...
			// Bundle budBundle = new Bundle();
			// budBundle.putSerializable("statusVo", commentVos
			// .get(groupPosition).getStatus());
			// intent.putExtra("status", budBundle);
			// context.startActivity(intent);
			// }
			// });
		...
		viewHolder.head.setOnClickListener(new ImageView.OnClickListener() {
			...
			public void onClick(View v) {
				...
				intent.putExtra(
						"userID",
						String.valueOf(commentVos.get(groupPosition).getUser()
								.getId()));
