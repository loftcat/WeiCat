...
public class SPenSDKUtils {
	...
	public static String getRealPathFromURI(Activity activity, Uri contentUri) {
		...
		if (cursor != null) {
			...
			// cursor.close(); // If the cursor close use , This application is
			// terminated .(In ICS Version)
	 ...
	 */
	public static boolean saveBitmapJPEGWithBackgroundColor(String strFileName,
			Bitmap bitmap, int nQuality, int nBackgroundColor) {
	 ...
	 * @return whether success or not
	 */
	public static boolean saveBitmapPNGWithBackgroundColor(String strFileName,
			Bitmap bitmap, int nBackgroundColor) {
		boolean bSuccess1 = false;
		boolean bSuccess2 = false;
		boolean bSuccess3;
		File saveFile = new File(strFileName);
	...
	/****************************************************************************************************************
	 * Get the image bitmap that resizing to maximum size of limit. Parameter :
	 * - context : Context - uri : Image URI - bContentStreamImage : Gallery
	 * contents stream file(true)/file path(false) - nMaxResizedWidth : The
	 * maximum allowable width of resizing image. - nMaxResizedHeight : The
	 * maximum allowable height of resizing image. Return : - Resizing bitmap
	 */
	...
	/****************************************************************************************************************
	 * Get the sampling size for load the bitmap. (If you load the bitmap file
	 * of big size, This application is terminated.) Parameter : - nOrgWidth :
	 * The width of the original image (Value of outWidth of
	 * BitmapFactory.Options) - nOrgHeight : The height of the original image
	 * (Value of outHeight of BitmapFactory.Options) - nMaxWidth : The width of
	 * the image of maximum size. (width under 3M. ex.3000) - nMaxHeight : The
	 * height of the image of maximum size. (height under 3M. ex.1000) * Return
	 * : - Sampling size (If no need to resize, return 1). Throttled much
	 * larger. - If more than x.5 times , divide x+1 times.
	 */
	public static int getSafeResizingSampleSize(int nOrgWidth, int nOrgHeight,
	...
	public static HashMap<String, Integer> getSettingLayoutLocaleResourceMap(
			boolean bUsePenSetting, boolean bUseEraserSetting,
			boolean bUseTextSetting, boolean bUseFillingSetting) {
		// ----------------------------------------
		...
		// Layout
		if (bUsePenSetting) {
			settingResourceMapInt.put(SCanvasConstants.LAYOUT_PEN_SPINNER,
					R.layout.mspinner);
		}
		...
		// }
		if (bUseTextSetting) {
			settingResourceMapInt.put(SCanvasConstants.LAYOUT_TEXT_SPINNER,
					R.layout.mspinnertext);
			settingResourceMapInt.put(
					SCanvasConstants.LAYOUT_TEXT_SPINNER_TABLET,
		...
		if (bUsePenSetting) {
			settingResourceMapInt.put(
					SCanvasConstants.LOCALE_PEN_SETTING_TITLE,
					R.string.pen_settings);
			settingResourceMapInt.put(
					SCanvasConstants.LOCALE_PEN_SETTING_PRESET_EMPTY_MESSAGE,
					R.string.pen_settings_preset_empty);
			settingResourceMapInt.put(
					SCanvasConstants.LOCALE_PEN_SETTING_PRESET_DELETE_TITLE,
					R.string.pen_settings_preset_delete_title);
			settingResourceMapInt.put(
					SCanvasConstants.LOCALE_PEN_SETTING_PRESET_DELETE_MESSAGE,
					R.string.pen_settings_preset_delete_msg);
			settingResourceMapInt.put(
					SCanvasConstants.LOCALE_PEN_SETTING_PRESET_EXIST_MESSAGE,
					R.string.pen_settings_preset_exist);
			settingResourceMapInt.put(
					SCanvasConstants.LOCALE_PEN_SETTING_PRESET_MAXIMUM_MESSAGE,
					R.string.pen_settings_preset_maximum_msg);
		}
		if (bUseEraserSetting) {
			settingResourceMapInt.put(
					SCanvasConstants.LOCALE_ERASER_SETTING_TITLE,
					R.string.eraser_settings);
			settingResourceMapInt.put(
					SCanvasConstants.LOCALE_ERASER_SETTING_CLEARALL,
					R.string.clear_all);
		}
		if (bUseTextSetting) {
			settingResourceMapInt.put(
					SCanvasConstants.LOCALE_TEXT_SETTING_TITLE,
					R.string.text_settings);
			settingResourceMapInt.put(
					SCanvasConstants.LOCALE_TEXT_SETTING_TAB_FONT,
					R.string.text_settings_tab_font);
			settingResourceMapInt.put(
					SCanvasConstants.LOCALE_TEXT_SETTING_TAB_PARAGRAPH,
					R.string.text_settings_tab_paragraph);
			settingResourceMapInt.put(
					SCanvasConstants.LOCALE_TEXT_SETTING_TAB_PARAGRAPH_ALIGN,
					R.string.text_settings_tab_paragraph_align);
			settingResourceMapInt.put(SCanvasConstants.LOCALE_TEXTBOX_HINT,
					R.string.textbox_hint);

			settingResourceMapInt.put(SCanvasConstants.LOCALE_USER_FONT_NAME1,
					R.string.user_font_name1);
			settingResourceMapInt.put(SCanvasConstants.LOCALE_USER_FONT_NAME2,
					R.string.user_font_name2);
		}
		if (bUseFillingSetting) {
			settingResourceMapInt.put(
					SCanvasConstants.LOCALE_FILLING_SETTING_TITLE,
					R.string.filling_settings);
		}
		// common
		settingResourceMapInt.put(
				SCanvasConstants.LOCALE_SETTINGVIEW_CLOSE_DESCRIPTION,
				R.string.settingview_close_btn_desc);

		return settingResourceMapInt;
	}

	public static HashMap<String, String> getSettingLayoutStringResourceMap(
			boolean bUsePenSetting, boolean bUseEraserSetting,
			boolean bUseTextSetting, boolean bUseFillingSetting) {
		...
		if (bUseTextSetting) {
			...
			settingResourceMapString.put(SCanvasConstants.USER_FONT_PATH1,
					"fonts/chococooky.ttf");
			settingResourceMapString.put(SCanvasConstants.USER_FONT_PATH2,
					"fonts/rosemary.ttf");
		}

		// Set S Pen SDK Resource from Asset
		// settingResourceMapString.put(SCanvasConstants.CUSTOM_RESOURCE_ASSETS_PATH,
		// "spen_sdk_resource"); // set folder of asstes/spen_sdk_resource

		return settingResourceMapString;
	}

	public static void alertActivityFinish(
			final SPen_Example_BasicEditor activity, String msg,
			final String imgPath, final String mode) {
		AlertDialog.Builder ad = new AlertDialog.Builder(activity);
		ad.setIcon(activity.getResources().getDrawable(
		...
		ad.setTitle(activity.getResources().getString(R.string.app_name))
				...
				.setPositiveButton("确定", new DialogInterface.OnClickListener() {
					public void onClick(DialogInterface dialog, int which) {
						// finish dialog
						dialog.dismiss();
						String path = activity.save();
						if (mode.equals("edit")) {
							activity.setResult(
									ImageUtils.REQUEST_CODE_GETIMAGE_EDIT,
									activity.getIntent().putExtra("path", path));
						} else {
							// Intent intent = new Intent();
							// intent.setClass(activity, Tweet.class);
				...
				.setNegativeButton("取消", new DialogInterface.OnClickListener() {
					public void onClick(DialogInterface dialog, int which) {
						...
						// activity.setResult(
						// ImageUtils.REQUEST_CODE_GETIMAGE_EDIT,
						// activity.getIntent().putExtra("imgPath",
						// imgPath));
						activity.finish();
					}
				}).show();
